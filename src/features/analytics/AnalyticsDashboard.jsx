...
export function AnalyticsDashboard({ logs, groups = [], setGroups, onOpenProfile, ollamaOnline, ollamaLoading, onOllamaPatternSummary, allStudents = DB.students }) {
    ...
    return (
      <div className="panel" style={{ padding: "14px", borderLeft: `3px solid ${s.color || 'var(--accent)'}` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "10px" }}>
          <div>
            <div className="privacy-blur" tabIndex={0} style={{ fontWeight: "700", fontSize: "14px", color: s.color, cursor: "pointer" }} onClick={() => onOpenProfile(id)}>{resolveLabel(s, "compact")}</div>
      ...
      <div className="panel" style={{ padding: "14px", borderLeft: "3px solid #a78bfa" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" }}>
          <div>
            ...
            <div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginTop: "4px" }}>{group.students.map(id => <span key={id} className="privacy-blur" tabIndex={0} style={{ fontSize: "10px", color: allStudents[id]?.color, background: allStudents[id]?.color + "15", padding: "1px 7px", borderRadius: "20px" }}>{resolveLabel(allStudents[id], "compact")}</span>)}</div>
          </div>
          <div style={{ display: "flex", gap: "4px" }}>
  ...
  return (
    <div>
      <div className="header"><div><h1>Analytics</h1><p className="teacher-subtitle">{filteredLogs.length} logs in range · {Object.keys(allStudents).length} students tracked</p></div></div>
      ...
      {tab === "overview" && (<div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
          ...
          <div className="panel" style={{ padding: "16px" }}><div style={{ fontSize: "13px", fontWeight: "600", color: "#e2e8f0", marginBottom: "10px" }}>Per Student</div>
            {Object.keys(allStudents).map(id => { const sl = logsFor([id]); return sl.length > 0 ? (<div key={id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 0", borderBottom: "1px solid #0f172a" }}><span className="privacy-blur" tabIndex={0} style={{ fontSize: "12px", color: allStudents[id].color, cursor: "pointer" }} onClick={() => onOpenProfile(id)}>{resolveLabel(allStudents[id], "compact")}</span><span style={{ fontSize: "12px", fontWeight: "600", color: "#8fa3c4" }}>{sl.length}</span></div>) : null; })}
          </div>
        </div>
      </div>)}

      {tab === "students" && (<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>{Object.keys(allStudents).map(id => <StudentAnalytics key={id} id={id} />)}</div>)}
      ...
      {tab === "groups" && (<div>
        ...
        {showGroupForm && (<div className="panel" style={{ padding: "16px", marginBottom: "16px" }}>
          ...
          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "10px" }}>
            {Object.entries(allStudents).map(([id, s]) => (<button key={id} className="privacy-blur" tabIndex={0} onClick={() => toggleGroupStu(id)} style={{ fontSize: "11px", padding: "4px 10px", borderRadius: "6px", border: `1px solid ${newGroupStudents.includes(id) ? s.color : s.color + "40"}`, cursor: "pointer", background: newGroupStudents.includes(id) ? s.color + "25" : "transparent", color: s.color, fontWeight: newGroupStudents.includes(id) ? "600" : "400" }}>{resolveLabel(s, "compact")}</button>))}
          </div>
